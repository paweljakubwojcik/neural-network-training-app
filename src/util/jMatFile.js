...
var matrix = function (start, step, sizesRem, extractor) {
    ...
    for (var i = 0; i < retArray.length; i++) {
        retArray[i] = doSomething(start + i * step, step * thisDim, sizesRem.slice(1), extractor)
    }
    return retArray
}
...
var numeric = function (re, im, size, logical, complex) {
    var mkExtractor = function () {
        if (logical) {
            return function (pos) {
                return Boolean(re[pos])
            }
        } else if (complex) {
            return function (pos) {
                return { re: re[pos], im: im[pos] }
            }
        } else {
            return function (pos) {
                return re[pos]
...
var interpMiMatrix = function (raw) {
    ...
    miMat.numel = miMat.size.reduce(function (a, b) {
        return a * b
    })
    // Convention: scalars cannot be empty, but vectors can be
    var isScalar = miMat.numel === 1
    var isVector = miMat.size.length === 2 && (miMat.size[0] <= 1 || miMat.size[1] <= 1)
    ...
    switch (matClass) {
        ...
        case 'mxCHAR_CLASS':
            ...
            if (isVector) {
                // Note: a scalar is always a vector also
                miMat.string = bytes2String(raw[3].data)
            }
            break
        case 'mxCELL_CLASS':
            // do cell format here
            var cellValues = raw.splice(3).map(function (a) {
                return a.data[0]
            })
        ...
        case 'mxSTRUCT_CLASS':
            // do struct here
            var fieldValues = raw.slice(5).map(function (a) {
                return a.data[0]
            })
        ...
        case 'mxOBJECT_CLASS':
            ...
            miMat.object = true
            var propValues = raw.slice(6).map(function (a) {
                return a.data[0]
            })
    ...
    if (isVector && !miMat.sparse) {
        ...
        // memory
        miMat.vector = miMat.empty
            ? []
            : miMat.size[0] === 1
            ? miMat.value[0]
            : miMat.value.map(function (cv) {
                  return cv[0]
              })
    }
    return miMat
...
var matConfig = {
    ...
    tagData: jBinary.Template({
        ...
        read: function (ctx) {
            ...
            switch (ctx.tag.type) {
                ...
                case 'miINT8':
                    elems = new Int8Array(b, s, m)
                    view.skip(m)
                    break
                case 'miUINT8':
                    elems = new Uint8Array(b, s, m)
                    view.skip(m)
                    break
                case 'miINT16':
                    elems = new Int16Array(b, s, m / 2)
                    view.skip(m)
                    break
                case 'miUINT16':
                    elems = new Uint16Array(b, s, m / 2)
                    view.skip(m)
                    break
                case 'miINT32':
                    elems = new Int32Array(b, s, m / 4)
                    view.skip(m)
                    break
                case 'miUINT32':
                    elems = new Uint32Array(b, s, m / 4)
                    view.skip(m)
                    break
                case 'miSINGLE':
                    elems = new Float32Array(b, s, m / 4)
                    view.skip(m)
                    break
                case 'miDOUBLE':
                    elems = new Float64Array(b, s, m / 8)
                    view.skip(m)
                    break
    ...
    bitLengthArray: jBinary.Template({
        ...
        setParams: function () {
            this.baseType = ['array', this.element]
        },
        read: function (ctx) {
            var items = []
            var view = this.binary.view
